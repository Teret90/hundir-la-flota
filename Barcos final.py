...
def crear_y_colocar_barco_valido(eslora, tablero):
    ...
    lista_barcos_validos.append(barco)
...
### hacemos una lista de tuplas donde la 0ª posicion de la tupla es la "eslora" y la 1ª posición "cantidad de barcos a crear" y se llama <<barcos_a_insertar>>
...
def comprobar_tocado(barco,tablero):
    for i in barco.posiciones:
        if tablero[i] == "X":
...
def pedir_coordenadas():
    disparo_jugador_x= 2
    int(input("Por favorintroduce tu coordenada x: "))
...
print("Empieza el turno del J1, si acierta sigue disparando hasta que tenga un fallo, entonces será el turno de la máquina")
